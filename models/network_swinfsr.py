...
# Adapted to support different input/output channel counts (e.g., in_chans=8, out_chans=3).
# -----------------------------------------------------------------------------------
...
class WindowAttention(nn.Module):
    ...
    def flops(self, N):
        ...
        flops += self.num_heads * N * (self.dim // self.num_heads) * N  # q@k
...
class PatchEmbed(nn.Module):
    """ Image to Patch Embedding (here it just flattens features; conv_first does the real projection). """
...
class PatchUnEmbed(nn.Module):
    """ Patch Unembedding: tokens -> feature map. """
...
class UpsampleOneStep(nn.Sequential):
    """Single step upsample (1 conv + PS) used in lightweight SR."""
...
class SwinFSR(nn.Module):
    r""" SwinIR with flexible in/out channels.

    Args:
        in_chans (int): # input channels (e.g., 8 = RGB+5 heatmaps)
        out_chans (int): # output channels (e.g., 3 = RGB)
        upscale: 2/3/4/8 for SR (1 for denoise/JPEG)
        upsampler: 'pixelshuffle' | 'pixelshuffledirect' | 'nearest+conv' | ''
        img_range: 1. or 255.
    """
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3, out_chans=3,
        ...
        # record for FLOPs/info
        self.num_in_ch = in_chans
        self.num_out_ch = out_chans
        ...
        # means: subtract only from RGB-shaped inputs; if guidance channels exist, use zero-mean
        rgb_mean = (0.4488, 0.4371, 0.4040)
        if in_chans == 3:
            self.mean_in = torch.Tensor(rgb_mean).view(1, 3, 1, 1)
        else:
            self.mean_in = torch.zeros(1, 1, 1, 1)  # broadcast to in_chans
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            # real-world SR (only supports x4 here)
            assert self.upscale == 4, 'nearest+conv only supports x4 currently.'
            self.conv_before_upsample = nn.Sequential(nn.Conv2d(embed_dim, num_feat, 3, 1, 1),
                                                      nn.LeakyReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        # mean shift
        self.mean_in = self.mean_in.type_as(x)
        self.mean_out = self.mean_out.type_as(x)
        x = (x - self.mean_in) * self.img_range
        ...
        x = x / self.img_range + self.mean_out

        # crop to original (unpadded) * upscale
        return x[:, :, :H * self.upscale, :W * self.upscale]

    def flops(self):
        # Rough estimate; not used in training.
        flops = 0
        ...
        for layer in self.layers:
            flops += layer.flops()
        flops += H * W * self.embed_dim * self.embed_dim * 9  # conv_after_body (approx)
        # upsample (if present)
...
if __name__ == '__main__':
    ...
    upscale = 8
    window_size = 8
    height = (128 // window_size + 1) * window_size  # LR height multiple of window
    width = (128 // window_size + 1) * window_size   # LR width multiple of window

    # Example: 8-ch input (RGB + 5 heatmaps), 3-ch output (RGB)
