...
def imread_uint(path, n_channels=3):
    """
    Load image from disk robustly and convert to uint8 RGB.
    Returns: HxWxC (C=1 or 3) or None if image failed to load.
    """
    if not os.path.isfile(path):
        print(f"[imread_uint] File not found: {path}")
        return None
    img = cv2.imread(path, cv2.IMREAD_UNCHANGED)
    if img is None:
        print(f"[imread_uint] cv2.imread failed (corrupted?): {path}")
        return None
    if n_channels == 1:
        if img.ndim == 3:
